...
void parse_map() {
	...
	node_t* n_distance = mk_re("distance", "^[1-9][0-9]*k?m", "i");
	
	node_t* n_el = nop(mk_seq("end line", mk_optional(mk_seq("comment", mk_str("#"), mk_re("truly_anything", "^[^\\n]*", "i"))), mk_nl()));

	node_t* n_ini_pair = mk_seq("ini_pair", n_name, nop(mk_str("=")), n_anything, n_el);

	node_t* n_place_contents = mk_any();
	node_t* n_contains_contents = mk_any();
	node_t* n_constraints_contents = mk_any();
	...
	node_t* n_place = mk_seq("place", mk_str("[place]"), n_el, mk_star("place_line", n_place_contents));
